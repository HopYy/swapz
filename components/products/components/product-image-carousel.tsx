    ...
    <div className="rounded-sm overflow-hidden relative">
      ...
      {images.length > 1 && hover && (   
        ...
        <div className="absolute bottom-4 right-0 left-0 flex justify-center">
          ...
          <div className="flex items-center justify-start gap-2 delay-200 duration-100 ease-in-out" style={{ transform: `translateX(${curr >= 4 ? `-${curr - 2}rem` : '0'})` }}>
              {images.map((_, index) => (
                <div 
                  key={index}
                  className={cn("transition-all min-w-2 max-w-2 w-2 h-2 bg-white rounded-full",
