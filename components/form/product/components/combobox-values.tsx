...
export const CategoriesBox: React.FC<ComboboxProps> = ({ value, disabled, onChange }) => {
    return (
        <ComboboxComponent
          ...
          disabled={disabled}
          onChange={onChange}
          comboboxFormValues={categories.map((route) => ({label: route.label.toLowerCase()}))}
        />
    )
  }
...
export const ConditionsBox: React.FC<ComboboxProps> = ({ value, disabled, onChange }) => {
    return (
      <ComboboxComponent
        ...
        disabled={disabled}
        onChange={onChange}
        comboboxFormValues={conditions.map((condition) => ({ label: condition.label }))}
      />
  )
}

export const ShippingBox: React.FC<ComboboxProps> = ({ value, disabled, onChange }) => {
  return (
    <ComboboxComponent
      ...
      disabled={disabled}
      onChange={onChange}
      comboboxFormValues={shipping_products.map(({ label }) => ({ label: label }))}
    />
)
}