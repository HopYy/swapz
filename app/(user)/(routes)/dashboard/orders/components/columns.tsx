...
export const Columns = () => {
    ...
    const columns = [
        ...
        columnHelper.accessor(row => `${row.order.firstName} ${row.order.lastName}`, {
            id: "customer",
            header: "Customer",
...
const ProductColumn = ({ info }: { info: any }) => (
    <div className="max-w-[30vw] flex items-center gap-4 overflow-hidden p-2 max-xl:hidden">
        ...
        />
        <h1 className="overflow-hidden overflow-ellipsis whitespace-nowrap">{info.getValue()}</h1>
    </div>
)

const Date = ({ info }: { info: any }) => (
    <div className="p-2 lg:p-3 max-xl:hidden">
        <h1 className="text-md text-gray-500 overflow-hidden overflow-ellipsis whitespace-nowrap">{format(info.getValue(), 'dd MMM, h:mma')}</h1>
    </div>
)

const Status = ({ info }: { info: any }) => (
    <div className="flex items-center gap-2 p-2 lg:p-3">
        <div className={cn("w-2 h-2 rounded-full", info.getValue() ? "bg-green-600" : "bg-red-600")} />
        <span className="text-gray-500 font-medium">{info.getValue() ? "Paid" : "Canceled"}</span>
    </div>
)

const Customer = ({ info }: { info: any }) => (
    <div className="p-2 lg:p-3">
        <h1 className="text-md text-gray-500 overflow-hidden overflow-ellipsis whitespace-nowrap">{info.getValue()}</h1>
    </div>
)
...
const Price = ({ info }: { info: any }) => (
    <div className="flex gap-2 p-2 lg:p-3">
        ...
        <h1 className="text-gray-700 text-md font-semibold overflow-hidden overflow-ellipsis whitespace-nowrap">{info.getValue()}</h1>
    </div>
)