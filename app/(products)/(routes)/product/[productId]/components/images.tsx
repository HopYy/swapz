...
export const Images: React.FC<ImagesProps> = ({ data, isSold }) => {
  ...
  return (
    <div
      className={cn(
        'flex-1 flex flex-col justify-end items-center gap-3',
        fullscreen && 'w-screen h-screen fixed inset-0 z-30 bg-white'
      )}
    >
      <div
        className={cn(
      ...
      {data.length > 1 && (
        <ScrollArea
          className={cn(
            'max-sm:w-screen sm:max-w-[600px] pb-2',
            fullscreen && 'hidden'
          )}
        >
