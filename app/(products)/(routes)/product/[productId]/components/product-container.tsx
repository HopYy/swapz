...
export const ProductContainer: React.FC<ProductContainer> = ({
  product,
  user,
}) => (
    ...
    <div className="flex-1 space-y-2 md:mx-4 lg:max-w-[450px]">
      ...
      <div className="space-x-3 py-5">
        ...
        <span className="text-sm font-bold">
          {
            conditions.find(({ query }) => query === product.condition)
              ?.description
          }
        </span>
      </div>
      ...
      <div className="px-4 border lg:rounded-md">
        ...
        <div className="border-b flex gap-4 items-center py-4">
          ...
          <span className="text-sm font-semibold">
            {
              shipping_products.find(({ label }) => label === product.shipping)
                ?.description
            }
          </span>
        </div>
        <div className="border-b flex gap-4 items-center py-4">
          ...
          <span className="text-sm font-semibold">
            {
              return_item.find(
                ({ available }) => available === product.returnItem
              )?.description
            }
          </span>
        </div>
        <div className="flex gap-4 items-center py-4">
          ...
          <span className="text-sm font-semibold">
            {product.sold
              ? 'Product is unavailable in store'
              : 'Product is still available in store'}
          </span>
        </div>
      </div>
    </div>
